function STAP_MCAP= function_STAP_MCAP(Data,fd,Rangegate_now,Rangegate_end,N_TrainData,N_timechannel,R_real,arry_num,Ss,Wq,K,CNR)
%% 
        %Data：当前景数据
        %fd:多普勒通道
...
%     end
    ...
    T = kron(Tt,eye(arry_num));%降维矩阵（局域二维联合导向矢量）
 %% 得到降维后的R阵  
    %用训练样本估计待检测通道杂波协方差矩阵
    Data_T = T'*Data;
...
%% 得到改善因子IF_3DT和最终的自适应权值W_3DT
    ...
    IF_MCAP(n) = SCNR_out/SCNR_in; 
    W_MCAP(:,n)=miu*T*W_T;
    Output(n)=abs(W_MCAP(:,n)'*Data(:,Rangegate_now));
end
STAP_MCAP.IF_MCAP=IF_MCAP;
STAP_MCAP.W_MCAP=W_MCAP;
STAP_MCAP.Output=Output;