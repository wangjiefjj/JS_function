...
%%%%%%%%%%%%%%%%%%%%%%载机参数%%%%%%%%%%%%%%%%%%%%%%%
u=398600.8e9;
airborne_parameter.H=800e3                %载机高度(m)
re=6378e3;                                 %地球半径
airborne_parameter.V=sqrt(u/(airborne_parameter.H+re))                 %载机速度（m/s)
%%%%%%%%%%%%%%%%%%%%%%信号参数%%%%%%%%%%%%%%%%%%%%%%%
...
Ve=465.1;%地球赤道速度
Deta=Ve/airborne_parameter.V*(1+airborne_parameter.H/Re);
phi_c=atan(Deta*sqrt(cos(alpha1)^2-cos(eta_i)^2)/(1-Deta*cos(eta_i)))%单基地偏航角 圆轨道
rho_c=sqrt(1+Deta^2*cos(alpha1)^2-2*Deta*cos(eta_i))%偏航幅度
%% 载入参数
...
%         F_t=function_Ft(cita,phi,cita0,phi0);                                                                                                                %发射方向图
        ...
        P_r=P_r0/r^4;                                                                                                                                              %反射功率
        Sigma=Fun_clutter_reflect_model(r,phi,airborne_parameter,radar_parameter,signal_parameter,entironment_parameter);   %雷达等效截面积
        ...
        clutter_freq_fluct=Fun_clutter_freq_fluct(clutter_freq_fluct_flag,reflect_cell_num,entironment_parameter,airborne_parameter,signal_parameter,radar_parameter);    %杂波信号的频率起伏
        
        Phase_rand=exp(j*pi*(1-2*rand(1,reflect_cell_num)));                                                                                %等距离环各单元格杂波信号的随机相位
        ...
        t_ph=exp(j*2*pi*(t_frq.'*(0:pulse_num-1)/fr));                                                                                            %时频相位
        ...
        for m=1:reflect_cell_num
            ...
            s_ph_azimuth=exp(-j*2*pi*s_frq_azimuth.'*(0:arry_col_num-1));                %空频方位维相位
            s_ph_pitch=exp(-j*2*pi*s_frq_pitch.'*(0:arry_row_num-1));                        %空频俯仰维相位
...
%             end
    ...
    if flag_noise == 1
        Z =randn(Sys_DOF,1);
        gauss_noise = sqrt(noise_parameter.Pn)*Z.*exp(j*unifrnd(-pi,pi,Sys_DOF,1));%产生高斯噪声
    else
