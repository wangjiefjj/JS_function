...
for i_rho = 1:length(rho)
    waitbar(i_rho/length(rho),h,sprintf([num2str(i_rho/length(rho)*100),'%%']));
    if Class==1%%均匀
        str_train = 'g';
     %%杂波协方差
   ...
   parfor i = 1:MC
     ...
     Accuracy_AIC1(i_rho) = count_AIC1/MC;
     Accuracy_GIC1(i_rho) = count_GIC1/MC;
     Accuracy_AICc1(i_rho) = count_AICc1/MC;
     Accuracy_ABIC1(i_rho) = count_ABIC1/MC;

     Accuracy_AIC2(i_rho) = count_AIC2/MC;
     Accuracy_GIC2(i_rho) = count_GIC2/MC;
     Accuracy_AICc2(i_rho) = count_AICc2/MC;
     Accuracy_ABIC2(i_rho) = count_ABIC2/MC;

     Accuracy_AIC3(i_rho) = count_AIC3/MC;
     Accuracy_GIC3(i_rho) = count_GIC3/MC;
     Accuracy_AICc3(i_rho) = count_AICc3/MC;
     Accuracy_ABIC3(i_rho) = count_ABIC3/MC;
end
toc
close(h)
% figure(1)
...
figure(2)
hold on
plot(rho,Accuracy_AIC2,'r-s','LineWidth',2)
plot(rho,Accuracy_GIC2,'g-o','LineWidth',2)
plot(rho,Accuracy_AICc2,'b-*','LineWidth',2)
plot(rho,Accuracy_ABIC2,'k-.','LineWidth',2)
title('主辅数据')
...
figure(3)
hold on
plot(rho,Accuracy_AIC3,'r-s','LineWidth',2)
plot(rho,Accuracy_GIC3,'g-o','LineWidth',2)
plot(rho,Accuracy_AICc3,'b-*','LineWidth',2)
plot(rho,Accuracy_ABIC3,'k-.','LineWidth',2)
title('主数据')
h_leg2 = legend('AIC','GIC','AICc','ABIC');
xlabel('K')
ylabel('Accuracy')
