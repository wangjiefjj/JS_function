...
for i_fc = 1:length(fc)
    waitbar(i_fc/length(fc),h,sprintf([num2str(i_fc/length(fc)*100),'%%']));
    if Class==1%%均匀
        str_train = 'g';
         %%杂波协方差
        Rc1 = fun_rho(rho,N,1,fc(i_fc));
        %     sigmaf = 0.03; %%杂波谱展宽
        ...
        Rc2 = Rc1;
        opt_train = 0;
        str=['Hom_fc_L',num2str(L),'.mat'];
        elseif Class == 2%%部分均匀
            str_train = 'g';
            opt_train = 2;
         %%杂波协方差
            Rc2 = fun_rho(rho,N,1,fc(i_fc));
        %     sigmaf = 0.03; %%杂波谱展宽
        %     rc =  exp(-1i*2*pi*nn*fc-2*(nn*pi*sigmaf).^2);
        %     Rc1 = CNRnum * toeplitz(rc);
            Rc2 = Rc2+ 1/CNRnum * eye(N) ;%+ eye(N)
            Rc1 = 10*Rc2;
            str=['Partial_fc_L',num2str(L),'.mat'];
        elseif Class == 3%%SIRP
            str_train = 'p';
   ...
   parfor i = 1:MC
     ...
     Accuracy_AIC1(i_fc) = count_AIC1/MC;
     Accuracy_GIC2_1(i_fc) = count_GIC2_1/MC;
     Accuracy_GIC4_1(i_fc) = count_GIC4_1/MC;
     Accuracy_AICc1(i_fc) = count_AICc1/MC;
     Accuracy_ABIC1(i_fc) = count_ABIC1/MC;

     Accuracy_AIC2(i_fc) = count_AIC2/MC;
     Accuracy_GIC2_2(i_fc) = count_GIC2_2/MC;
     Accuracy_GIC4_2(i_fc) = count_GIC4_2/MC;
     Accuracy_AICc2(i_fc) = count_AICc2/MC;
     Accuracy_ABIC2(i_fc) = count_ABIC2/MC;

     Accuracy_AIC3(i_fc) = count_AIC3/MC;
     Accuracy_GIC2_3(i_fc) = count_GIC2_3/MC;
     Accuracy_GIC4_3(i_fc) = count_GIC4_3/MC;
     Accuracy_AICc3(i_fc) = count_AICc3/MC;
     Accuracy_ABIC3(i_fc) = count_ABIC3/MC;
end
toc
close(h)
% figure(1)
...
figure(2)
hold on
plot(fc,Accuracy_AIC2,'r-s','LineWidth',2)
plot(fc,Accuracy_GIC2_2,'g-o','LineWidth',2)
plot(fc,Accuracy_GIC4_2,'g-.o','LineWidth',2)
plot(fc,Accuracy_AICc2,'b-*','LineWidth',2)
plot(fc,Accuracy_ABIC2,'k-.','LineWidth',2)
title('主辅数据')
...
hold on
plot(fc,Accuracy_AIC2,'r-s','LineWidth',2)
plot(fc,Accuracy_GIC2_3,'g-o','LineWidth',2)
plot(fc,Accuracy_GIC4_3,'g-.o','LineWidth',2)
plot(fc,Accuracy_AICc2,'b-*','LineWidth',2)
plot(fc,Accuracy_ABIC2,'k-.','LineWidth',2)
title('主数据')
h_leg2 = legend('AIC','GIC2','GIC4','AICc','ABIC');
