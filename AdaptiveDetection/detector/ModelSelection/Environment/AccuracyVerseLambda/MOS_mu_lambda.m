...
for i_lambda = 1:length(lambda)
    for i_mu = 1:length(mu)   
        tt =((i_lambda-1)*length(lambda)+i_mu)/length(mu)/length(lambda);
        waitbar(tt,h,sprintf([num2str(tt*100),'%%']));
        count_AIC1 = 0;
        count_ABIC1 = 0;
        count_GIC1 = 0;
        count_AICc1 = 0;
        ...
        parfor i = 1:MC
            warning off
            %%产生数据
            [Train,tauk] = fun_TrainData(str_train,N,L,Rc1,lambda(i_lambda),mu(i_mu),opt_train);%%产生的训练数据,协方差矩阵为rouR的高斯杂波
            [x0,tau0] = fun_TrainData('g',N,1,Rc2,lambda(i_lambda),mu(i_mu),opt_train); % 接收信号仅包括杂波和噪声
            x0 =x0+a*p;
            %%s函数计算 
            %只用辅助数据时
        ...
        Accuracy_AIC1(i_lambda,i_mu) = count_AIC1/MC;
        Accuracy_GIC1(i_lambda,i_mu) = count_GIC1/MC;
        Accuracy_AICc1(i_lambda,i_mu) = count_AICc1/MC;
        Accuracy_ABIC1(i_lambda,i_mu) = count_ABIC1/MC;

        Accuracy_AIC2(i_lambda,i_mu) = count_AIC2/MC;
        Accuracy_GIC2(i_lambda,i_mu) = count_GIC2/MC;
        Accuracy_AICc2(i_lambda,i_mu) = count_AICc2/MC;
        Accuracy_ABIC2(i_lambda,i_mu) = count_ABIC2/MC;

        Accuracy_AIC3(i_lambda,i_mu) = count_AIC3/MC;
        Accuracy_GIC3(i_lambda,i_mu) = count_GIC3/MC;
        Accuracy_AICc3(i_lambda,i_mu) = count_AICc3/MC;
        Accuracy_ABIC3(i_lambda,i_mu) = count_ABIC3/MC;
    end
end
...
mesh(X,Y,Accuracy_GIC2)
mesh(X,Y,Accuracy_AICc2)
mesh(X,Y,Accuracy_ABIC2)
title('主辅数据')
