...
for i_fd = 1:length(fd)
    ...
    %%%%%%%%%%%训练数据产生%%%%%%%%%%%%%0%
        ...
        R_KA2 = zeros(N,N);
        for ii = 1:before-N+1
            x_tt = Zhh(index-before+ii-1:index-before+ii+N-2,Range);
            R_KA1 = R_KA1+fun_NSCMN(x_tt)/(before-N+1);
            R_KA2 = R_KA2+x_tt*x_tt'/(before-N+1);
        end
%         R_KA = eye(N);
        Train1 = Zhh(index:index+N-1,Range-L/2+1:Range-1);
        Train2 = Zhh(index:index+N-1,Range+1:Range+L/2+1);
        Train = [Train1,Train2];%%产生的训练数据,协方差矩阵为rouR的高斯杂波
        x0 = Zhh(index:index+N-1,Range) ; % 接收信号仅包括杂波和噪声
    % % % %     协方差估计
...
for i_fd=1:length(fd)
    waitbar(i_fd/length(fd),h,sprintf(['检测概率计算: ', num2str(i_fd/length(fd)*100),'%%']));
    s = exp(-1i*2*pi*nn*fd(i_fd)).'/sqrt(N); %%%%%% 系统导向矢量
    for i=offset:MonteCarloPd+offset
        %%%%%%%%%%%训练数据产生%%%%%%%%%%%%%0%
        R_KA1 = zeros(N,N);
        R_KA2 = zeros(N,N);
        for ii = 1:before-N+1
            x_tt = Zhh(i-before+ii-1:i-before+ii+N-2,Range);
            R_KA1 = R_KA1+fun_NSCMN(x_tt)/(before-N+1);
            R_KA2 = R_KA2+x_tt*x_tt'/(before-N+1);
        end
%         R_KA = eye(N);
        Train1 = Zhh(i:i+N-1,Range-L/2+1:Range-1);
        Train2 = Zhh(i:i+N-1,Range+1:Range+L/2+1);
        Train = [Train1,Train2];%%产生的训练数据,协方差矩阵为rouR的高斯杂波
...
grid on
str = ['PdFd2_PFA_',num2str(L),'Second',str_IPIX_t,'IPIX','.mat'];
save (str); 
