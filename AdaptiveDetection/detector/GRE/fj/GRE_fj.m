...
tic
for m=1:length(fj)
    waitbar(m/length(fj),h,sprintf([num2str(m/length(fj)*100),'%%']));
    %% 各种协方差
    %%杂波协方差
    fc = 0;
    sigmaf = 0.03; %%杂波谱展宽
...
figure(2);
hold on
plot(fj,Pd_SGLRT_mc,'k-o','linewidth',2,'MarkerSize',2)
plot(fj,Pd_SRAO_mc,'r-x','linewidth',2,'MarkerSize',10)
plot(fj,Pd_SWALD_mc,'b-x','linewidth',2,'MarkerSize',10)
legend('GLRT','RAO','WALD')
xlabel('fj','FontSize',20)
