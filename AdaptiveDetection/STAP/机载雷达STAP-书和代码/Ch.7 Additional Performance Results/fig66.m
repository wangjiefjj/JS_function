%% Figure 66.(a) SINR loss for example system, â=2.6, ó_õ = 0.2 m/sec, no mislainment.
%
%
...
b2 = zeros(K2,Nc);
Rc = zeros(M*N,M*N);
Rcsub2 = zeros(K2*N,K2*N);
 
gammac =  exp(-(kc^2*Tr^2*(0:M-1).^2)/2);
gammac_sub =  exp(-(kc^2*Tr^2*(0:K2-1).^2)/2);
...
Kt =  3;                                        % sub-CPI length (fixed). 
Ks = 5;                                        % sub-apperture length or number of beams used. 
M1 = M - Kt +1;                          % Number of possible sub-CPI's.
...
LSINRpostTSN = SINR30/SNRo;
  ...
  %% Plot the SINR Losses:
     figure('NumberTitle', 'off','Name', ... 
                'Figure 66(a) SINR loss for example system, â=2.6, no misalinment, ó_õ = 0.2 m/sec');
    ...
    plot(fd,10*log10(LSINR30),'g','LineWidth',1.5);
     ...
     legend('Optimum Fully Adaptive','Elem-Pre (54)','Elem-Post (54)',...
                   'Beam-Pre TSN (15)','Beam-Post TSN (15)','Location','South')
     grid on;
